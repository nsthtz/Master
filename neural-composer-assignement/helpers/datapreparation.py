...
def generate_round(model,tag,n,k=1,init=None):
    ...
    for i in range(n//k):
        ...
        print("Output", i, init)
        lstm_out = torch.cat((lstm_out, init))
...
def gen_music(model,length=1000,init=None,composer=0,fs=5):
    if(init is None):
        song, lstm_out=generate_round(model, torch.LongTensor([composer]).unsqueeze(1),length,1)
    else:
        song, lstm_out=generate_round(model, torch.LongTensor([composer]).unsqueeze(1),length,1,init)
...
def gen_music_initkeys(model,length=1000,initkeys=40,composer=0,fs=5):
    ...
    res = ( song.squeeze(1).detach().numpy()).astype(int).T
    visualize_piano_roll(res,fs)
    return embed_play_v1(res,fs)
    #return song
...
def gen_music_seconds(model,init,composer=0,fs=5,gen_seconds=10,init_seconds=5):
    ...
    res = ( song.squeeze(1).detach().numpy()).astype(float).T
    visualize_piano_roll(res,fs)
    return embed_play_v1(res,fs)
